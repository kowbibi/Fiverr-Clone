...
class User extends Database {
     ...
     * @param bool $is_administrator Whether the user is an administrator.
     * @return bool True on success, false on failure.
     */
    public function registerUser($username, $email, $password, $contact_number, $is_client = 1, $is_administrator = 0) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $sql = "INSERT INTO fiverr_clone_users (username, email, password, is_client, is_administrator, contact_number) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            $this->executeNonQuery($sql, [$username, $email, $hashed_password, $is_client, $is_administrator, $contact_number]);
            return true;
    ...
    public function loginUser($email, $password) {
        $sql = "SELECT user_id, username, password, is_client, is_administrator FROM fiverr_clone_users WHERE email = ?";
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            $_SESSION['is_administrator'] = (bool)$user['is_administrator'];
            return true;
        }
        return false;
    }
    ...
    public function isAdministrator() {
        $this->startSession();
        return isset($_SESSION['is_administrator']) && $_SESSION['is_administrator'];
    }
     ...
     * @return bool True on success, false on failure.
     */
    public function createAdminUser($username, $email, $password, $contact_number, $is_client = 1, $is_administrator = 1) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $sql = "INSERT INTO fiverr_clone_users (username, email, password, is_client, is_administrator, contact_number) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            $this->executeNonQuery($sql, [$username, $email, $hashed_password, $is_client, $is_administrator, $contact_number]);
